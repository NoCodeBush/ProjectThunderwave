...
const JobDetails: React.FC = () => {
  ...
  const actionButtons = [
    ...
    {
      title: 'Image/Scan',
      description: 'Take photos and apply scanner effects',
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      action: () => {
        navigate(ROUTES.IMAGE_SCAN.replace(':jobId', job.id))
      }
    },
    {
      title: 'Document Upload',
      description: 'Upload and manage documents (PDF, Word, Excel, etc.)',
      icon: (
        <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      action: () => {
        navigate(ROUTES.DOCUMENT_UPLOAD.replace(':jobId', job.id))
      }
    }
  ]
